...
if [ -n "${NO_VOLUME_MOUNT}" ]; then
    ...
    docker run -e GITHUB_BASE_REPO_URL=$GITHUB_BASE_REPO_URL -e GITHUB_PULL_REQUEST=$GITHUB_PULL_REQUEST -e GITHUB_HEAD_REPO_BRANCH=$GITHUB_HEAD_REPO_BRANCH --rm balrogtest test $@
else
    echo "Running tests with volume mount"
    docker run -e GITHUB_BASE_REPO_URL=$GITHUB_BASE_REPO_URL -e GITHUB_PULL_REQUEST=$GITHUB_PULL_REQUEST -e GITHUB_HEAD_REPO_BRANCH=$GITHUB_HEAD_REPO_BRANCH --rm -v `pwd`:/app balrogtest test $@
fi
