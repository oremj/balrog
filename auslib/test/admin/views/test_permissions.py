...
class TestPermissionsAPI_JSON(ViewTest, JSONTestMixin):
    ...
    def testPermissionsCollection(self):
        ...
        self.assertEqual(ret.status_code, 200)
        self.assertEqual(json.loads(ret.data), dict(admin=dict(options=None, data_version=1, system=False)))

    def testPermissionGet(self):
        ret = self._get('/users/bill/permissions/admin')
        self.assertEqual(ret.status_code, 200)
        self.assertEqual(json.loads(ret.data), dict(options=None, data_version=1, system=False))
    ...
    def testPermissionPut(self):
        ...
        self.assertEqual(query.execute().fetchone(), ('admin', 'bob', None, False, 1))
    ...
    def testPermissionsPost(self):
        ...
        self.assertEqual(r[0], ('admin', 'bill', None, False, 2))

    def testPermissionUrl(self):
        ret = self._put('/users/cathy/permissions/releases/:name', data=dict(system=True))
        self.assertStatusCode(ret, 201)
        self.assertEqual(ret.data, json.dumps(dict(new_data_version=1)), "Data: %s" % ret.data)
        query = db.permissions.t.select()
        query = query.where(db.permissions.username=='cathy')
        query = query.where(db.permissions.permission=='/releases/:name')
        self.assertEqual(query.execute().fetchone(), ('/releases/:name', 'cathy', None, True, 1))
    ...
    def testPermissionPutWithOption(self):
        ...
        self.assertEqual(query.execute().fetchone(), ('/rules', 'bob', json.dumps(dict(product='fake')), False, 1))
    ...
    def testPermissionModify(self):
        ...
        query = query.where(db.permissions.username=='bob')
        query = query.where(db.permissions.permission=='/releases/:name')
        self.assertEqual(query.execute().fetchone(), ('/releases/:name', 'bob', json.dumps(dict(product='different')), False, 2))
